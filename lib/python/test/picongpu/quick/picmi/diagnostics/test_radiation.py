...
def test_generated_setup_includes_particle_filters_param():
    # radiation.param uses picongpu::particles::filter::random_number, which is
    # defined in particleFilters.param.  Radiation.x.cpp includes radiation.param
    # unconditionally (ENABLE_OPENPMD), so without this include every build fails
    # with `name followed by "::" must be a class or namespace name` -- even for
    # simulations that never use radiation.
    for filtered in (False, True):
        rendered = _render(filtered=filtered)
        assert '#include "picongpu/param/particleFilters.param"' in rendered.radiation_param
        assert "using picongpu::particles::filter::random_number;" in rendered.radiation_param
